...
namespace sdds {
	...
	TennisLog::TennisLog(const char* file) {
		...
		for (auto i = 0U; i < num_of_matches; ++i) {
	...
	TennisLog::TennisLog(const TennisLog& right) {
		*this = right;
	}

	TennisLog::TennisLog(TennisLog&& right) noexcept {
		*this = std::move(right);
	}

	TennisLog& TennisLog::operator=(TennisLog&& right) noexcept {
		if (this != &right) {
			delete[] matches;
			num_of_matches = 0;
			matches = right.matches;
			num_of_matches = right.num_of_matches;
			right.matches = nullptr;
			right.num_of_matches = 0;
		}
		return *this;
	}

	TennisLog& TennisLog::operator=(const TennisLog& right) {
		if (this != &right) {
			const auto max_sz = right.operator size_t();
			delete[] matches;
			matches = new TennisMatch[max_sz + 1];
			num_of_matches = right.num_of_matches;
			for (auto i = 0U; i < max_sz; ++i) {
				matches[i] = right.matches[i];
			}
		}
		return *this;
	}
	...
	TennisMatch TennisLog::operator[](size_t index) const {
		if (const auto sz = this->operator size_t(); !sz || index > sz) {
			return TennisMatch{};
		}
